param(
    [ValidateSet('Check','Install','Open','Run')][string]$Action = 'Check',
    [switch]$Consent,
...
function Install-Tool($Name) {
    ...
    Write-Host "正在准备：$($entry.label)"
    if (!(Test-Path $archive) -or (Get-FileHash $archive -Algorithm SHA256).Hash -ne $entry.sha256) {
        Invoke-WebRequest -UseBasicParsing $entry.url -OutFile $archive
    }
...
function Install-Environment {
    ...
    & $tools.uv pip install --python $python 'playwright==1.63.0'
    if ($LASTEXITCODE -ne 0) { throw '浏览器连接工具安装失败，可重试继续。' }
    $tools.python = $python
...
try {
    ...
    if ($Action -eq 'Check') {
        ...
        @{ configuration=$saved; loginSaved=(Test-Path (Join-Path $root 'douyin-session.protected')); aiSaved=[bool]([Environment]::GetEnvironmentVariable('DASHSCOPE_API_KEY', 'User') -or $env:DASHSCOPE_API_KEY); supported=$true; directory=$root; missing=@($missing); environmentPrepared=(Test-Path (Join-Path $root 'toolchain.json')); next='首次默认配置全部功能；用户可选择暂时只下载。安装前请获得同意。'; pythonEnvironment='独立 Python 3.11 与浏览器连接工具；缺失时自动下载' } | ConvertTo-Json
        exit 0
    }
